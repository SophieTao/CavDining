...
def meal(request, cafe_id):
		...
		post1 = urllib.parse.urlencode(resp).encode('utf-8')
		context = {'resp': resp}
		if auth != None:
			data ={}
			...
			producer.send('recommendation-topic', json.dumps(listing_info).encode('utf-8'))
		#return HttpResponse(urllib.parse.urlencode(listing_info).encode('utf-8'))
		return render(request, 'api/meal.html', context)
...
def search_listing(request):
	...
	try:
		...
		return HttpResponse("Please create new listing first")
	resp = json.loads(resp_json)
	if resp['ok']:
